...
# this can be swapped with shard implementation
...
myRCTD <- spacexr::create.RCTD(puck,reference,max_cores = 4,test_mode = F, UMI_min = 20,CELL_MIN_INSTANCE = 25)
myRCTD <- spacexr::run.RCTD(myRCTD, doublet_mode = "doublet")
results <- myRCTD@results

write.csv(results$results_df,"../../../../data/Mouse_Embryo/Mouse_Embryo_RCTD_nuclei.csv")
...
# alternatively if not filtering, it is possible to change the default value of the CELL_MIN_INSTANCE parameter
myRCTD2 <- spacexr::create.RCTD(puck2,reference,max_cores = 4,test_mode = F, UMI_min = 20,CELL_MIN_INSTANCE = 25)
myRCTD2 <- spacexr::run.RCTD(myRCTD2, doublet_mode = "doublet")
results2 <- myRCTD2@results

write.csv(results2$results_df,"../../../../data/Mouse_Embryo/Mouse_Embryo_RCTD_cells.csv")
...
write.csv(df_class_RCTD, paste0(path, "../../../../data/Mouse_Embryo/Mouse_Embryo_RCTD_filters.csv"))